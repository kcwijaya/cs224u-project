...
from data_util import batch_data_nn
from tensorflow.python.ops import variable_scope as vs
from tensorflow.python.ops.rnn_cell import DropoutWrapper

class RNNModel(Model): 
	def __init__(self, depth = 5, lr = 0.001, max_length=70, regularization_factor = 0.001, keep_prob = 0.5, batch_size=200, hidden_size=150):
		self.lr = lr
	...
	def build_graph(self): 
		...
		for i in range(0, self.depth): 
			...
			inputs = out 
		logits = tf.contrib.layers.fully_connected(out, num_outputs=1, activation_fn=None)
		logits = tf.squeeze(logits, axis=[2])
		logits, prob_dist = self.masked_softmax(logits, 1) 
		print("preds", out.get_shape())
	...
	def predict(self, X, batch_size=None): 
		...
		for i in range(0, len(X), batch_size): 
			preds[i:i+batch_size] = np.argmax(self.session.run(
				self.prob_dist,
				{
	...
	def fit(self, train_dataset, train_label_dataset, num_epochs=10):
		...
		validation_set = batches[num_train_batches:]
		...
		for epoch in range(1, num_epochs+1):
			...
			for batch, (X_batch, mask_batch, y_batch) in enumerate(train_batches):
				train_loss, _, train_accuracy = self.session.run((self.loss, self.train_op, self.metrics), {
				...
				progbar.update(batch+1, [('Train Loss', train_loss), ('Accuracy', train_accuracy)])

			print("Epoch complete. Calculating validation loss...")

			valid_loss, _, valid_accuracy = self.session.run((self.loss, self.train_op, self.metrics), {
			...
			print('Validation Loss: {0:.4f} {1} Validation Accuracy {2:.4f} {3}'.format(valid_loss, '*', valid_accuracy, '*'))
			print('Training Loss: {0:.4f} {1} Training Accuracy {2:.4f} {3}'.format(train_loss, '*', train_accuracy, '*'))
		print("Done training.")
